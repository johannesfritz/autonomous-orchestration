...
The audit trail is stored in `inbox/audit_log.jsonl` (JSON Lines format):
...
1. **Read audit log:**
   ```bash
   cat inbox/audit_log.jsonl
   ```

2. **Filter by plan ID (if provided):**
   ```bash
   grep "PLAN-2025-XXX" inbox/audit_log.jsonl
   ```
...
All agents MUST log to `inbox/audit_log.jsonl`:
