...
```bash
1. Read state file: 00 Inbox/system_state.json
...
2. Atomic write (prevents corruption):
   - Write to: 00 Inbox/system_state.json.tmp
   - Rename to: 00 Inbox/system_state.json
...
- User overrides a decision that used the pattern
- Plan prioritized by pattern fails or gets rejected
    ...
    "priority_patterns": [
      {
        ...
        "last_applied": "2025-01-15T10:00:00Z"
      }
    ],
    "high_scrutiny_paths": [
      "src/auth/",
...
When showing portfolio status or making decisions, indicate when patterns are applied:

```
Prioritization Decision: PLAN-2025-005
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
📚 Learned Pattern Applied:
   - "customer-facing" keyword detected (80% confidence)
   - Boosting priority by 1 level

Final Priority: high → critical (pattern-boosted)

Reasoning: This plan contains "customer-facing" keyword which has been
prioritized 5 times in past decisions. Applying learned pattern as tiebreaker.
```

### Learning Transparency

Always be transparent about learning:

1. **Log when patterns are created:**
   ```
   📚 New pattern learned: "customer" keyword → priority boost
      Based on: 5 user overrides
      Initial confidence: 50%
   ```

2. **Log when patterns are applied:**
   ```
   📚 Applied learned pattern: "auth" files → high scrutiny
      Confidence: 75%
      Will flag PR for manual review
   ```

3. **Log when patterns decay:**
   ```
   📚 Pattern confidence reduced: "urgent" keyword
      Old confidence: 60%
      New confidence: 45%
      Reason: User overrode pattern-based decision
   ```
